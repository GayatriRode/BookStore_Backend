...
app.post('/logout', async (req, res) => {
  ...
  try {
    // Find the user by userId and update the logout time
    const user = await User.findByIdAndUpdate(userId, { $set: { logoutTime: new Date() } }, { new: true });
...
// Add this endpoint to fetch orders by customer ID
app.get('/api/orders/customer/:customerId', async (req, res) => {
  const { customerId } = req.params;

  try {
    // Fetch orders for the validated customer
    const orders = await Order.find({ customer: customerId }).populate('book');
      ...
      { $unwind: '$customerDetails' },
      {
        $lookup: {
          ...
          pipeline: [
            ...
            {
              $match: {
                $expr: { $eq: ['$authors.books._id', '$$bookId'] }
              }
            },
            {
              $project: {
                _id: 0,
                bookName: '$authors.books.name'
              }
            }
          ],
          as: 'bookDetails'
...
app.get('/:customerId', async (req, res) => {
  ...
  try {
    ...
    // Fetch orders for the validated customer
    const orders = await Order.find({ customer: customerId }).populate('book', 'title author'); 
    res.json(orders);
  } catch (error) {
    console.error('Error fetching orders:', error);
    res.status(500).json({ error: 'Error fetching orders. Please try again later.' });
...
app.post('/api/feedbacks', async (req, res) => {
  try {
    ...
    // Assuming Feedback is a Mongoose model
